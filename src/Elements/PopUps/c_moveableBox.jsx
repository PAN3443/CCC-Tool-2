...
class CMoveableBox extends Component {
    ...
    return (
      ...
      >
        <div
          ...
          className="cl_row"
          style={{ padding: "1vh 2vw", background: "var(--bg-point-out)", zIndex: "101", cursor: "move" }}
        >
          <h3 className="cl_notSelectable" style={{ margin: "auto", color: "var(--bg-app)" }}>
            {this.props.header}
          </h3>
        </div>
  ...
  startDrag = (event) => {
    ...
    this.documentHeight = Math.max(documentBody.scrollHeight, documentBody.offsetHeight, documentHtml.clientHeight, documentHtml.scrollHeight, documentHtml.offsetHeight);
    this.documentWidth = Math.max(documentBody.scrollWidth, documentBody.offsetWidth, documentHtml.clientWidth, documentHtml.scrollWidth, documentHtml.offsetWidth);
  };
